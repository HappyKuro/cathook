...
public:
  ...
  int get_ammo_count(int ammo_type) {
    ...
    if (ammo_type < 0 || ammo_type >= max_ammo_types) {
      return 0;
    }
    ...
    auto* ammo = reinterpret_cast<int*>(reinterpret_cast<uintptr_t>(this) + offset);
  ...
  bool taunt_force_move_forward() {
    ...
    return *reinterpret_cast<bool*>(reinterpret_cast<uintptr_t>(this) + allow_move_during_taunt_offset + 1);
  }
  ...
  float taunt_force_move_forward_speed() {
    ...
    return *reinterpret_cast<float*>(reinterpret_cast<uintptr_t>(this) + allow_move_during_taunt_offset + 4);
  }
  ...
  bool copy_cached_bones(matrix_3x4* bone_to_world_out, int max_bones, int* bone_count_out = nullptr) {
    if (bone_count_out != nullptr) {
      *bone_count_out = 0;
    }
    if (bone_to_world_out == nullptr || max_bones <= 0 || max_bones > 128) {
      return false;
    }
  ...
  Vec3 get_bone_pos(int bone_num) {
    matrix_3x4 bone_to_world_out[128]{};
    int bone_count = 0;
    if (bone_num >= 0 &&
        copy_cached_bones(bone_to_world_out, 128, &bone_count) &&
        bone_num < bone_count) {
      return Vec3{
        bone_to_world_out[bone_num].mat[0][3],
        bone_to_world_out[bone_num].mat[1][3],
        bone_to_world_out[bone_num].mat[2][3]
      };
    }
  ...
  bool get_hitbox_center(int hitbox_id, Vec3* center_out, int* bone_out = nullptr) {
    ...
    int bone_count = 0;
    if (!copy_cached_bones(bone_to_world, 128, &bone_count) || hitbox->bone >= bone_count) {
      return false;
    }
  ...
  bool is_disguised(void) {
    return in_cond(TF_COND_DISGUISED) || in_cond(TF_COND_DISGUISED_AS_DISPENSER);
  }

  bool is_taunting(void) {
    return in_cond(TF_COND_TAUNTING);
  }

  bool is_dead_ringer_active(void) {
    return in_cond(TF_COND_FEIGN_DEATH);
  }

  bool is_vaccinator_resistant(void) {
    return in_cond(TF_COND_MEDIGUN_UBER_BULLET_RESIST) ||
      ...
      in_cond(TF_COND_MEDIGUN_SMALL_BULLET_RESIST) ||
      in_cond(TF_COND_MEDIGUN_SMALL_BLAST_RESIST) ||
      in_cond(TF_COND_MEDIGUN_SMALL_FIRE_RESIST);
  }

  bool is_sentry_buster(void) {
    const char* model_name = get_model_name();
    return model_name != nullptr && std::strstr(model_name, "sentry_buster") != nullptr;
  }
