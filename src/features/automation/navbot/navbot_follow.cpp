...
bool nav_edge_valid_follow(nav_edge_id edge_id)
{
  return edge_id.from_area != 0;
}

bool crumb_range_contains_nav_edge(const path_result& path, size_t first_crumb, size_t last_crumb)
{
  if (path.crumbs.empty() || first_crumb >= path.crumbs.size())
  {
    return false;
  }

  const auto end = std::min(last_crumb, path.crumbs.size() - 1);
  for (auto index = first_crumb; index < end; ++index)
  {
    if (nav_edge_valid_follow(path.crumbs[index].edge_id))
    {
      return true;
    }
  }
...
bool navbot_auto_engage_can_block_jump(Player* localplayer, Weapon* weapon)
{
  ...
  {
    return config.aimbot.sniper_auto_scope;
  }
...
nav_edge_id find_failed_edge(const path_result& path, size_t crumb_index)
{
  if (crumb_index < path.crumbs.size() && nav_edge_valid_follow(path.crumbs[crumb_index].edge_id))
  {
    return path.crumbs[crumb_index].edge_id;
  }

  if (crumb_index > 0 && crumb_index - 1 < path.crumbs.size() && nav_edge_valid_follow(path.crumbs[crumb_index - 1].edge_id))
  {
    return path.crumbs[crumb_index - 1].edge_id;
  }
  ...
  size_t crumb_index)
{
  ...
  result.failed_edge = find_failed_edge(path, crumb_index);
...
size_t navbot_follow::find_skip_ahead_crumb(Player* localplayer, const Vec3& local_origin) const
{
  ...
  {
    if (crumb_range_contains_nav_edge(active_path_, current_crumb_index_, crumb_index))
    {
      continue;
    }
  ...
  {
    if (crumb_range_contains_nav_edge(active_path_, current_crumb_index_, segment_index + 1))
    {
      continue;
    }
...
follower_tick_result navbot_follow::tick(Player* localplayer, user_cmd* user_cmd, float current_time)
{
  ...
  {
    fill_failure_result(result, follower_failure_reason::no_progress, active_path_, current_crumb_index_);
    return result;
  }

  if (current_time - current_crumb_start_time_ > blocked_fail_time)
  {
    fill_failure_result(result, follower_failure_reason::blocked, active_path_, current_crumb_index_);
    return result;
  }
