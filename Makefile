...
FUNCHOOK_LIBS = $(FUNCHOOK_DIR)/libfunchook.a $(FUNCHOOK_DIR)/libdistorm.a
...
prepare-permissions:
	@mkdir -p "$(OUTPUT_DIR)" "obj" "$(FUNCHOOK_BUILD_DIR)"
	@needs_repair=0; \
	[ -w "$(OUTPUT_DIR)" ] || needs_repair=1; \
	[ -w "obj" ] || needs_repair=1; \
	if { [ ! -f "$(FUNCHOOK_DIR)/libfunchook.a" ] || [ ! -f "$(FUNCHOOK_DIR)/libdistorm.a" ]; } && [ ! -w "$(FUNCHOOK_BUILD_DIR)" ]; then needs_repair=1; fi; \
	if [ "$$needs_repair" -eq 1 ]; then \
		echo "Build directories are not writable; attempting to repair ownership..."; \
		sudo -n chown -R "$$(id -u):$$(id -g)" "$(OUTPUT_DIR)" "obj" "$(FUNCHOOK_BUILD_DIR)" || { \
			echo "Error: build directories are owned by another user and sudo access is required." >&2; \
			echo "Run: sudo chown -R $$(id -un):$$(id -gn) $(OUTPUT_DIR) obj $(FUNCHOOK_BUILD_DIR)" >&2; \
			exit 1; \
	}; \
	fi

funchook-libs: prepare-permissions
	@if [ -f "$(FUNCHOOK_DIR)/libfunchook.a" ] && [ -f "$(FUNCHOOK_DIR)/libdistorm.a" ]; then \
		echo "Using existing Funchook/diStorm static libraries."; \
	else \
